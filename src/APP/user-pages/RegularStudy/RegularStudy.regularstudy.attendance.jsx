...
export default function RegularStudyAttendance() {
  ...
  useEffect(() => {
    if (originalAttendanceData.length > 0) {
      ...
      console.log(
        'attendanceId 25 데이터:',
        originalAttendanceData.filter((item) => item.attendanceId === 25)
      );
  ...
  const fetchChallengeRewardLog = async () => {
    try {
      const response = await request.get(
        '/challenge/reward/log?logType=ACQUIRED'
      );
    ...
    <itemS.Container>
      ...
      <itemS.BtnContainer>
        {showCertificationBtn &&
          !isChallengeRewardMode &&
          hasValidAttendanceData && (
            <itemS.CertificationBtn onClick={openAuthModal}>
              출석 인증하기
            </itemS.CertificationBtn>
          )}
        {isChallengeRewardMode && (
          <itemS.CertificationBtn onClick={applyChallengeReward}>
            적용하기
          </itemS.CertificationBtn>
        )}
      </itemS.BtnContainer>
