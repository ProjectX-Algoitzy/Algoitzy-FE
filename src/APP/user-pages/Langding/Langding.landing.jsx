  ...
  useEffect(() => {
    const getRecentGeneration = async () => {
      try {
        ...
        const response = await axios.get(
          `${process.env.REACT_APP_API_URL}/generation/max`
        );
    ...
    const getStudyCount = async () => {
      try {
        ...
        const response = await axios.get(
          `${process.env.REACT_APP_API_URL}/study/count`
        );
      ...
      try {
        ...
        if (response["isSuccess"]) {
          ...
          localStorage.setItem(
            ...
            response.result.regularStudyMemberYn
          );
  ...
  return (
    <itemS.HomeWrap>
      <itemS.NormalWrap>
        ...
        <itemS.SecondSentence>
          {detailStudyCount}개의 스터디가 당신을 기다리고 있어요!
        </itemS.SecondSentence>
        <Link
        ...
        >
          <itemS.BtnStudyApply>
            {detailRecentGeneration}기 스터디 지원하기
          </itemS.BtnStudyApply>
        </Link>
      </itemS.NormalWrap>
      <itemS.ImgWrap>
