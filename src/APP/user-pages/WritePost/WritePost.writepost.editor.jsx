...
export default function Editor({
  ...
  const applyMarkdownSyntax = (syntax) => {
    ...
    if (syntax === "link") {
      const { top, left } = editorView.coordsAtPos(
  ...
  const handleLinkInsert = () => {
    ...
    editorView.dispatch(
      editorView.state.changeByRange((range) => ({
        ...
        range: EditorSelection.range(
          range.from + linkTextStart,
          range.from + linkTextEnd
        ),
      }))
    );
    setLinkURL("");
  ...
  const deleteFile = async (file) => {
    ...
    try {
      if (file.onlyS3 == true) {
        response = await request.delete("/s3", {
  ...
  const handlePostSubmit = async () => {
    ...
    try {
      ...
      if (response.isSuccess) {
        ...
        alert(
          boardId ? "게시글이 수정되었습니다." : "게시글이 등록되었습니다."
        );
        navigate(-1); // 이전 페이지로 이동
      } else {
  ...
  return (
    <Styled.LeftContainer>
      <Styled.InnerEditorContainer ref={editorRef} isScrolling={isScrolling}>
        ...
        <Styled.Toolbar>
          <button onClick={() => applyMarkdownSyntax("heading1")}>
            <img src="/img/toolbar_H1.svg" alt="Heading 1" />
          </button>
          <button onClick={() => applyMarkdownSyntax("heading2")}>
            <img src="/img/toolbar_H2.svg" alt="Heading 2" />
          </button>
          <button onClick={() => applyMarkdownSyntax("heading3")}>
            <img src="/img/toolbar_H3.svg" alt="Heading 3" />
          </button>
          <span>|</span>
          <button onClick={() => applyMarkdownSyntax("bold")}>
            <img src="/img/toolbar_bold.svg" alt="Bold" />
          </button>
          <button onClick={() => applyMarkdownSyntax("italic")}>
            <img src="/img/toolbar_italic.svg" alt="Italic" />
          </button>
          <button onClick={() => applyMarkdownSyntax("strikethrough")}>
            <img src="/img/toolbar_strikethrough.svg" alt="Strikethrough" />
          </button>
          <span>|</span>
          <button onClick={() => applyMarkdownSyntax("blockquote")}>
            <img src="/img/toolbar_blockquote.svg" alt="Blockquote" />
          </button>
          ...
          <input
            ...
            accept=".zip, .pdf, .hwp, .hwpx, .doc, .docx, .ppt, .pptx, .xls, .xlsx, .txt"
          />
          <button onClick={() => applyMarkdownSyntax("link")}>
            <img src="/img/toolbar_link.svg" alt="Link" />
          </button>
          ...
          />
          <button onClick={() => applyMarkdownSyntax("code")}>
            <img src="/img/toolbar_code.svg" alt="Code" />
          </button>
        </Styled.Toolbar>
