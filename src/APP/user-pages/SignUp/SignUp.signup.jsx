...
export default function Signup() {
  ...
  // 비밀번호 확인 border 색상
  const [pwdConfirmborderColor, setPwdConfirmBorderColor] =
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const errorMessage =
        ...
        error.response?.data?.result?.password ||
  ...
  const handleConfirmEmail = async () => {
    ...
    try {
      const response = await axios.post(
        `${process.env.REACT_APP_API_URL}/sign-up/email`,
        requestData
      );
      console.log("response", response.data);
      if (response.data["isSuccess"]) {
  ...
  const handleSubmitEmail = async () => {
    ...
    try {
      ...
      if (response.data["isSuccess"]) {
        ...
        setEmailMessage(
          "이메일로 인증번호가 발송되었습니다. 확인되지 않은 경우 스팸함을 확인해주세요."
        );
      } else {
        ...
        <itemS.InnerContainer>
          ...
          <div>
            ...
            <itemS.LIContainer>
              ...
              <itemS.CodeMessage style={{ color: `${emailMessageColor}` }}>
                {emailMessage}
              </itemS.CodeMessage>
            ...
            <itemS.Message style={{ color: `${emailCodeColor}` }}>
              {emailCodeMessage}
            </itemS.Message>
          </div>
          <itemS.Btn
