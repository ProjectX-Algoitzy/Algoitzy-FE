    ...
    try {
      ...
      if (accessToken) {
        ...
        response = await request.get(
          `/board?searchKeyword=${searchKeyword}&category=${selectedTab}&sort=${sortType}&page=${
            currentPage + 1
          }&size=${itemsPerPage}`
        );
      } else {
      ...
      const responseData = accessToken ? response : response.data;
      if (responseData.isSuccess) {
        console.log("게시글 목록 조회 성공");
        setPosts(responseData.result.boardList);
        setTotalPages(Math.ceil(responseData.result.totalCount / itemsPerPage));
      } else {
        console.error("게시글 목록 조회 실패:", responseData);
      }
    } catch (error) {
      ...
      <itemS.Container>
        <itemS.InnerContainer>
          ...
          <itemS.PaginationContainer>
            ...
            {isRegularMember ? (
              <itemS.WriteBtn onClick={handleWriteClick}>
                + 글쓰기
              </itemS.WriteBtn>
            ) : (
              <itemS.BlankBtn></itemS.BlankBtn>
