...
export default function EditMyInfo() {
  ...
  // 비밀번호 border 색상
  const [newPwdBorderColor, setNewPwdBorderColor] =
    useState("1px solid #CFCFCF");
  // 비밀번호 확인 border 색상
  const [newPwdConfirmBorderColor, setNewPwdConfirmBorderColor] =
    useState("1px solid #CFCFCF");
  ...
  const fetchMyInfo = async () => {
    try {
      ...
      if (response.isSuccess) {
        ...
        if (
          !NameRegex.test(response.result.name) &&
          response.result.name.trim().length > 0
        ) {
  ...
  const handlePhoneNumberChange = (value) => {
    ...
    if (numericValue.length > 3 && numericValue.length <= 7) {
      ...
      formattedValue = `${numericValue.slice(0, 3)}-${numericValue.slice(
        3,
        7
      )}-${numericValue.slice(7, 11)}`; // 010-xxxx-xxxx 로 변경
    }
  ...
  const handleConfirmHandle = async () => {
    ...
    try {
      const response = await axios.post(
        `${process.env.REACT_APP_API_URL}/sign-up/handle`,
        requestData
      );
  ...
  const handleConfirmPhone = async () => {
    ...
    try {
      const response = await axios.post(
        `${process.env.REACT_APP_API_URL}/sign-up/phone-number`,
        requestData
      );
      console.log("response", response.data);
      if (response.data["isSuccess"]) {
  ...
  const handleSubmitSMS = async () => {
    ...
    try {
      const response = await axios.post(
        `${process.env.REACT_APP_API_URL}/sms/certification`,
        requestData
      );
      console.log("response", response.data);
      if (response.data["isSuccess"]) {
      ...
      setPhoneMessageColor("#DC4A41"); // Red
      setPhoneMessage(
        error.response?.data?.result?.phoneNumber ||
        ...
        <itemS.InnerContainer>
          ...
          <div>
            ...
            <itemS.LIContainer>
              ...
              <itemS.EmailBox type="text" value={email} readOnly />
            </itemS.LIContainer>
            <itemS.LIContainer>
            ...
            <itemS.Message style={{ color: `${handleColor}` }}>
              {handleMessage}
            </itemS.Message>
              ...
              <>
                ...
                <itemS.LIContainer>
                  <itemS.InputBox
                    ...
                    onChange={(e) =>
                      handleNewPasswordConfirmChange(e.target.value)
                    }
            ...
            <itemS.LIContainer>
              ...
              <itemS.CodeMessage style={{ color: `${phoneMessageColor}` }}>
                {phoneMessage}
              </itemS.CodeMessage>

              <itemS.InputConfirmTimerBoxWrapper>
                <itemS.InputConfirmTimerBox
                  maxlength="10"
                  type="text"
                  placeholder="인증코드"
                  value={SMSCode}
            ...
            <itemS.Message style={{ color: `${SMSColor}` }}>
              {SMSMessage}
            </itemS.Message>
          </div>
          <itemS.Btn
